...
def finetune_model_a_dual_corrected(
    ...
    print(f"   ℹ️  Clinical states already included in State-Action pairs")
    ...
    # Save config
    config = {
        "model_type": "Model A - Chuyên khoa (CORRECTED)",
        "architecture": "Dual-Encoder (Two-Tower)",
        "state_encoder": {
            "base_model": "all-mpnet-base-v2",
            "pretraining": "None",
            "finetuning_data": f"clinical_traces ({len(training_examples)} State-Action pairs)"
        },
        "action_encoder": {
            "base_model": "all-mpnet-base-v2",
            "pretraining": "None",
            "finetuning_data": f"clinical_traces ({len(training_examples)} pairs)"
        },
        "batch_size": batch_size,
        "epochs": num_epochs
    }
...
def finetune_model_b_dual_corrected(
    ...
    print("=" * 70)
    print("🌟 Model B: Fine-tuning DUAL-ENCODER (Toàn diện)")
    print("=" * 70)
    print("CORRECTED LOGIC:")
    print("  - StateEncoder: Base → Fine-tune (Clinical)")
    ...
    # Save config
    config = {
        "model_type": "Model B - Toàn diện (CORRECTED)",
        "architecture": "Dual-Encoder (Two-Tower)",
        "state_encoder": {
            "base_model": "all-mpnet-base-v2",
            "pretraining": "None (States không có trong ADP+T1)",
            "finetuning_data": f"clinical_traces ({len(training_examples)} State-Action pairs)"
        },
        "action_encoder": {
            "base_model": "all-mpnet-base-v2",
